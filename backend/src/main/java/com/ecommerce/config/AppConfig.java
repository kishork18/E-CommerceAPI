	...
	public SecurityFilterChain configration(HttpSecurity http) throws Exception {
		http.sessionManagement(
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers("/swagger-ui*/**", "/v3/api-docs/**").permitAll()
						.requestMatchers("user/signin", "/user/register").permitAll()
						.requestMatchers(HttpMethod.PATCH,"/user/updateuser").hasAnyRole("ADMIN").anyRequest().authenticated())
...
//		http.authorizeHttpRequests(auth -> auth.requestMatchers("user/findAll", "/user/register").permitAll()
//			.requestMatchers(HttpMethod.PATCH,"/user/updateuser").hasAnyRole("ADMIN").anyRequest().authenticated())
//		
//		.csrf(c -> c.disable())
